...
/**
 * Replaces the domain in the set-cookie response with the proxy domain.
 * 
 * @param cookie {string} set-cookie header
 * @param domain {string} what the domain should be
 */
function fixCookieDomain(cookie, domain) {
  if (cookie instanceof Array) return cookie.map(function(cookie) {
    return fixCookieDomain(cookie, domain);
  });
...
function onProxyResponse(req, res, response) {
  ...
  if (response.headers['set-cookie']) {
    response.headers['set-cookie'] = fixCookieDomain(response.headers['set-cookie'], '.' + requestState.proxyHost);
  }
...
var getHandler = exports.getHandler = function(options) {
  var corsAnywhere = {
    ...
    allowedOrigins: /^$/,     // A regexp to test if origin os allowed.
  ...
  return function(req, res, proxy) {
    ...
    if (!corsAnywhere.allowedOrigins.test(req.headers['origin'])) {
      // Origin not allowed
      res.writeHead(403, 'Origin not allowed', cors_headers);
